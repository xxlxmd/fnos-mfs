...
`category.create=mfs` 的意思是 most free space，也就是“剩余空间最多”。当 App 在聚合目录里创建新文件时，mergerfs 会从选中的底层目录里，挑当前剩余空间绝对值最大的那块盘写入。
...
默认规则下，新文件会先写到硬盘 C，因为它剩余空间最多。当硬盘 C 降到接近 1000G 可用时，硬盘 B 也可能开始被选中。当大盘都降到接近 500G 可用时，硬盘 A 才会更常被选中。这个规则追求的是“剩余空间接近”，不是“使用百分比接近”。
